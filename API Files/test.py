...
def call_llm_with_chain(runnable, user_input, customer_name, campaign_name):
    # Call the runnable with all required variables
    ...
    # Optionally add external tools (e.g., Google Search)
    tools = []
    if SERPAPI_API_KEY:
        ...
        tools.append(Tool(
            name="Google Search",
            func=search.run,
            description="Useful for answering questions requiring real-time information from the web."
        ))

    # If tools are available, initialize an agent
    if tools:
        ...
        return conversation, agent, customer_name, campaign_name
    return conversation, None, customer_name, campaign_name